...
import { fetch_connections, fetch_path, mass_fetch_connections } from '../../utils/api/api';
...
const ConnectionsGame = () => {
	...
	const [currShortestPath, setCurrShortestPath] = useState<{ length: number; path: graphData['nodes'] }>({ length: -1, path: [] });
	...
	return (
		<div className={css.connectionsGame}>
			...
			<ScoreBar
				...
				resetTime={resetTime}
				currShortestPath={currShortestPath.length}
				shortestPath={shortestPath}
			></ScoreBar>
			{playerHistoryPlayerId !== '' && (
				<PlayerHistory player_id={playerHistoryPlayerId} open={playerHistoryPlayerId !== ''} onClose={() => setPlayerHistoryPlayerId('')} />
			)}
		</div>
	);
};
