...
export const translateFilters = (filterShemas: filterSchema[] | undefined): Filter[] => {
	...
	return filterShemas.map(filter => {
		...
		if ('league_id' in filter) {
			return { type: 'league', code: filter.league_id, name: filter.name, image: filter.img_ref.replace('tiny', 'big') };
		}
		...
		if ('player_id' in filter) {
			return { type: 'player', code: String(filter.player_id), name: filter.name, image: filter.img_ref.replace('medium', 'big') };
		}
